...
public class MMBannerLayout: UICollectionViewLayout {
    ...
    private var radius: CGFloat{
        get {
        return self.collectionView!.calculate.totalCount > 1 ? angle*CGFloat.pi/180 : 0 
        }
    }
    ...
    private var _isInfinite = true {
        didSet {
            if self.collectionView!.calculate.totalCount > 1 {
            _isInfinite = true
            let cycleStart = self._isInfinite ? twoDistance*CGFloat(self.collectionView!.calculate.totalCount*100000) : 0
    ...
    public var autoPlayStatus: AutoPlayStatus = .play(duration: 4) {
        didSet {
            timer?.invalidate()
            switch autoPlayStatus {
            case .none:
                timer = nil
                self.setCurrentIndex(currentIdx)
