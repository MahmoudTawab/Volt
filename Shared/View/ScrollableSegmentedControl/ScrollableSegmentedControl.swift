...
@objc public class ScrollableSegmentedControl: UIControl {
    ...
    private var longestTextWidth:CGFloat = ControlWidth(15.0)
    ...
    private class BaseSegmentCollectionViewCell: UICollectionViewCell {
        static let textPadding:CGFloat = ControlWidth(15.0)
        static let imageToTextMargin:CGFloat = ControlWidth(14.0)
        static let imageSize:CGFloat = ControlWidth(14.0)
        static let defaultFont = UIFont.systemFont(ofSize: ControlWidth(14))
        ...
        var UnderlineWidth:NSLayoutConstraint?
        var UnderlineWidthAnimate:NSLayoutConstraint?
        var normalAttributedTitle:NSAttributedString?
        var highlightedAttributedTitle:NSAttributedString?
        var selectedAttributedTitle:NSAttributedString?
        ...
        private func configureConstraints() {
            if let underline = underlineView {
                ...
                UnderlineWidth = underline.widthAnchor.constraint(equalTo: contentView.widthAnchor)
                UnderlineWidth?.isActive = true
                
                UnderlineWidthAnimate = underline.widthAnchor.constraint(equalTo: contentView.widthAnchor, multiplier: 1/3)
                UnderlineWidthAnimate?.isActive = true
            }
        }
...
//        }
        ...
        override var isSelected: Bool {
            didSet {
            self.underlineView?.isHidden = !self.isSelected
            UIView.animate(withDuration: 0.3, animations: {
            self.UnderlineWidth?.isActive = self.isSelected
            self.UnderlineWidthAnimate?.isActive = !self.isSelected
            self.layoutIfNeeded()
            }) { (isDone) in
            UIView.animate(withDuration: 0.3, animations: {
            self.UnderlineWidthAnimate?.isActive = !self.isSelected
...
//        }
        ...
        func animateSelected() {
            UIView.animate(withDuration: 0.3, animations: {
            self.titleLabel.frame.origin.y -= 2
            }) { (isDone) in
            UIView.animate(withDuration: 0.3, animations: {
            self.titleLabel.frame.origin.y += 2
            })
